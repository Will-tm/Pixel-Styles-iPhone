...
{
    CGFloat s = 1.0f;
    if([[UIScreen mainScreen] respondsToSelector:@selector(scale)]) s = [[UIScreen mainScreen] scale];
    
    UIGraphicsBeginImageContextWithOptions(size, YES, s);
...
- (UIImage*)imageByScalingAndCroppingForSize:(CGSize)targetSize
{
    ...
    UIGraphicsBeginImageContextWithOptions(targetSize, YES, s);
...
{
    CGFloat width = overlayImage.size.width;
    CGFloat height = overlayImage.size.height;
    ...
    {
        bounds.size.height = size.width - 2.0;
        bounds.size.width = bounds.size.height * ratio;
    }
    
    CGFloat offset = (size.height - bounds.size.height)/2.0 - 0.5;
    
    CGFloat s = 1.0f;
    if([[UIScreen mainScreen] respondsToSelector:@selector(scale)]) s = [[UIScreen mainScreen] scale];
    
    UIGraphicsBeginImageContextWithOptions(size, YES, s);
    [iconImage drawInRect:CGRectMake(0.0, 0.0, size.width, size.height)];
    [overlayImage drawInRect:CGRectMake(1.0, offset, bounds.size.width, bounds.size.height+1)];
